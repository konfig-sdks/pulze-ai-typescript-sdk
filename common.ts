...
import { readableStreamToString, PulzeAiError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PulzeAiError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PulzeAiError(e, e.response?.data)
                }
                if (innerError instanceof PulzeAiError) {
                    // Got "PulzeAiError" from the above try block
                    throw innerError;
                }
